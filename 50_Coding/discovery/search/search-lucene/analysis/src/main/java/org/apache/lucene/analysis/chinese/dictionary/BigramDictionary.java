...
/**
 * ChineseAnalyzer Bigram dictionary.
 */
public class BigramDictionary extends AbstractDictionary {
	...
	private void load() throws IOException, ClassNotFoundException {
		...
		InputStream input = this.getClass().getClassLoader().getResourceAsStream("bigramdict.mem");
		...
		PiceneAssert.notNull(input, "The bigramdict.mem file does not exist!");
	...
	private File getFileFromDataHome() {
		...
		String filePath = home + "data" + File.separator + "mem" + File.separator + "bigramdict.mem";
			...
			for (i = GB2312_FIRST_CHAR; i < GB2312_FIRST_CHAR + CHAR_NUM_IN_FILE; i++) {
				...
				while (j < cnt) {
					...
					buffer[0] = ByteBuffer.wrap(intBuffer)
							.order(ByteOrder.LITTLE_ENDIAN).getInt();// frequency
					dctFile.read(intBuffer);
					buffer[1] = ByteBuffer.wrap(intBuffer)
							.order(ByteOrder.LITTLE_ENDIAN).getInt();// length
					dctFile.read(intBuffer);
