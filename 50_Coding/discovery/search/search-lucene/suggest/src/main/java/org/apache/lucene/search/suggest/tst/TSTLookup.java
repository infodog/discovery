...
public class TSTLookup extends Lookup {
     ...
     *
     * @see #build(org.apache.lucene.search.spell.TermFreqIterator)
     */
    public TSTLookup() {
    }
    ...
    public void build(TermFreqIterator tfit) throws IOException {
        ...
        while ((spare = tfit.next()) != null) {
            ...
            //TODO Trick?
            stores.add(charsSpare.toString() + " ");

            //TODO Discovery
            //chinese pinyin
            try {
                String pinyin = StringUtil.getPinYin(charsSpare.toString());
                if (pinyin != null && !pinyin.equals(charsSpare)) {
                    tokens.add(pinyin);
                    //TODO Trick?
                    stores.add(charsSpare.toString() + " ");
                    vals.add(Long.valueOf(tfit.weight()));
        ...
        autocomplete.balancedTree(tokens.toArray(), stores.toArray(), vals.toArray(), 0, tokens.size() - 1, root);
    }
    ...
    public boolean add(CharSequence key, Object value) {
        autocomplete.insert(root, key, key, value, 0);
    ...
    public List<LookupResult> lookup(CharSequence key, boolean onlyMorePopular, int num) {
        ...
        if (onlyMorePopular) {
            ...
            for (TernaryTreeNode ttn : list) {
                ...
                queue.insertWithOverflow(new LookupResult(ttn.store, ((Number) ttn.val).longValue()));
            }
            ...
            for (int i = 0; i < maxCnt; i++) {
                ...
                res.add(new LookupResult(ttn.store, ((Number) ttn.val).longValue()));
            }
        }
        return res;
    }
