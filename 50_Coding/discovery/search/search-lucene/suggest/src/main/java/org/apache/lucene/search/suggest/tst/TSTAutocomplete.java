 ...
 *
 * @see org.apache.lucene.search.suggest.tst.TernaryTreeNode
 */
public class TSTAutocomplete {
     ...
     */
    public void balancedTree(Object[] tokens, Object[] stores, Object[] vals, int lo, int hi,
                             TernaryTreeNode root) {
        if (lo > hi) return;
        int mid = (lo + hi) / 2;
        //TODO Discovery
        root = insert(root, (String) tokens[mid], (String) stores[mid], vals[mid], 0);
        balancedTree(tokens, stores, vals, lo, mid - 1, root);
        balancedTree(tokens, stores, vals, mid + 1, hi, root);
    }
     ...
     */
    public TernaryTreeNode insert(TernaryTreeNode currentNode, CharSequence s, CharSequence store,
                                  Object val, int x) {
        ...
        if (currentNode == null) {
            ...
            currentNode = newNode;
            if (x < s.length() - 1) {
                currentNode.eqKid = insert(currentNode.eqKid, s, store, val, x + 1);
            } else {
                currentNode.token = s.toString();
                currentNode.val = val;
                currentNode.store = store.toString();
                return currentNode;
            }
        } else if (currentNode.splitchar > s.charAt(x)) {
            currentNode.loKid = insert(currentNode.loKid, s, store, val, x);
        } else if (currentNode.splitchar == s.charAt(x)) {
            if (x < s.length() - 1) {
                currentNode.eqKid = insert(currentNode.eqKid, s, store, val, x + 1);
            } else {
            ...
            currentNode.hiKid = insert(currentNode.hiKid, s, store, val, x);
        }
        return currentNode;
    }
