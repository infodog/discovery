...
public class SpellChecker implements java.io.Closeable {
  ...
  public static final String F_WORD = "word";
    ...
    //TODO Discovery
    public static final String CHINESE_WORD = "chinese.word";
   ...
   * @throws java.io.IOException if Spellchecker can not open the directory
   */
   ...
   * @throws java.io.IOException if there is a problem opening the index
   */
   ...
   * @throws org.apache.lucene.store.AlreadyClosedException if the Spellchecker is already closed
   * @throws  java.io.IOException if spellchecker can not open the directory
   */
  ...
  /**
   * The accuracy (minimum score) to be used, unless overridden in {@link #suggestSimilar(String, int, org.apache.lucene.index.IndexReader, String, org.apache.lucene.search.spell.SuggestMode, float)}, to
   ...
   * @throws java.io.IOException if the underlying index throws an {@link java.io.IOException}
   * @throws org.apache.lucene.store.AlreadyClosedException if the Spellchecker is already closed
   * @return String[]
   *
   * @see #suggestSimilar(String, int, org.apache.lucene.index.IndexReader, String, org.apache.lucene.search.spell.SuggestMode, float)
   */
  ...
  /**
   * Calls {@link #suggestSimilar(String, int, org.apache.lucene.index.IndexReader, String, org.apache.lucene.search.spell.SuggestMode, float)
  ...
  public String[] suggestSimilar(String word, int numSug, IndexReader ir,
    ...
    try {
      ...
      if (suggestMode == SuggestMode.SUGGEST_ALWAYS) {
        ...
        if (this.chinese) {
            try {
                word = StringUtil.getPinYinEx(word);
  ...
  //    System.out.println("HITS: " + hits.length());
      ...
      for (int i = 0; i < stop; i++) {
          ...
          //TODO Discovery
          if (this.chinese) {
              sugWord.string = indexSearcher.doc(hits[i].doc).get(CHINESE_WORD); // get orig word
              ...
              // edit distance
              String py = indexSearcher.doc(hits[i].doc).get(F_WORD);
   ...
   * @param config {@link org.apache.lucene.index.IndexWriterConfig} to use
        ...
        terms: while ((currentTerm = iter.next()) != null) {
          ...
          String word = currentTerm.utf8ToString();
            ...
            if (this.chinese) {
                ...
                try {
                    pinyin = StringUtil.getPinYinEx(word);
                } catch (BadHanyuPinyinOutputFormatCombination badHanyuPinyinOutputFormatCombination) {
                    badHanyuPinyinOutputFormatCombination.printStackTrace();
                }
                len = pinyin.length();
          ...
          // ok index the word
          Document doc = createDocument(word, getMin(len), getMax(len), this.chinese, pinyin);
  ...
  private static Document createDocument(String text, int ng1, int ng2,boolean chinese, String pinyin) {
    //TODO Discovery
    ...
    // checked for rebuild (and stored for retrieval). Doesn't need norms or TF/pos
      ...
      //chinese
      if (chinese) {
          Field f = new StringField(F_WORD, pinyin, Field.Store.YES);
          Field c = new StringField(CHINESE_WORD, text, Field.Store.YES);
          doc.add(f); // orig term
          doc.add(c);
          addGram(pinyin, doc, ng1, ng2);
      } else {
...
//    addGram(text, doc, ng1, ng2);
   ...
   * @throws java.io.IOException if the close operation causes an {@link java.io.IOException}
   * @throws org.apache.lucene.store.AlreadyClosedException if the {@link org.apache.lucene.search.spell.SpellChecker} is already closed
   */
  @Override
   ...
   * @throws java.io.IOException f there is a low-level IO error
   */
  ...
  /**
   * Returns <code>true</code> if and only if the {@link org.apache.lucene.search.spell.SpellChecker} is
   * closed, otherwise <code>false</code>.
   * 
   * @return <code>true</code> if and only if the {@link org.apache.lucene.search.spell.SpellChecker} is
   *         closed, otherwise <code>false</code>.
   */
  boolean isClosed(){
    return closed;
