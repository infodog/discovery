 ...
 * <li>A finite state automaton ({@link org.apache.lucene.util.fst.FST}) is constructed from the input. The
 * root node has arcs labeled with all possible weights. We cache all these
 * arcs, highest-weight first.</li>
 * </ul>
 * 
 * <p>
 * At runtime, in {@link org.apache.lucene.search.suggest.fst.FSTCompletion#lookup(CharSequence, int)},
...
public class FSTCompletionBuilder {
  ...
  /**
   * Scratch buffer for {@link #add(org.apache.lucene.util.BytesRef, int)}.
   */
  ...
  /**
   * Creates an {@link org.apache.lucene.search.suggest.fst.FSTCompletion} with default options: 10 buckets, exact match
   * promoted to first position and {@link InMemorySorter} with a comparator obtained from
   * {@link org.apache.lucene.util.BytesRef#getUTF8SortedAsUnicodeComparator()}.
   */
   ...
   *          in {@link #add(org.apache.lucene.util.BytesRef, int)} and must be smaller than the number
   *          given here.
   *          
   * @param sorter
   *          {@link org.apache.lucene.search.suggest.fst.BytesRefSorter} used for re-sorting input for the automaton.
   ...
   *          
   *          See the description of this parameter in {@link org.apache.lucene.util.fst.Builder}'s constructor.
