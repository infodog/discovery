...
/**
 * JSON Columns Response Writer
 */
public class JSONColsResponseWriter implements InsightResponseWriter {
    ...
    public void write(Writer writer, IndexSchema schema, SolrParams params, NamedList values, ReturnFields returnFields, int offset, int limit) throws IOException {
        JSONWriter w = new JSONWriter(writer, schema, params, values, returnFields, offset, limit);
        try {
            String[] chart_fields = params.getParams(InsightParams.INSIGHT_CHART_FIELDS);

            NamedList insight = (NamedList) values.get("insight");

            String[] fields;
            if (chart_fields == null || chart_fields.length == 0) {
                fields = (String[]) insight.get("fields");
            } else {
                fields = chart_fields;
            }

            List<NamedList> rows = (List<NamedList>) insight.get("rows");

            List<List> columns = getColumns(fields, rows);

            //add time span
            if ("_time".equals(fields[0])) {
                String[] temp = new String[fields.length + 1];
                for (int i = 0; i < fields.length; i++) {
                    temp[i] = fields[i];
                }
                temp[fields.length] = "_span";
                fields = temp;
                List<String> column = new ArrayList<String>(columns.get(0).size());
            ...
            values.add("init_offset", 0);
            values.add("review", false);
    ...
    private List<List> getColumns(String[] fields, List<NamedList> rows) {
        List<List> columns = new ArrayList<List>();
        for (String field : fields) {
            List<String> column = new ArrayList<String>();
            for (NamedList row : rows) {
                column.add((String) row.get(field));
            }
            columns.add(column);
        }
...
class JSONWriter extends InsightTextResponseWriter {
    ...
    public JSONWriter(Writer writer, IndexSchema schema, SolrParams params, NamedList values, ReturnFields returnFields, int offset, int limit) {
        super(writer, schema, params, values, returnFields, offset,limit);
        namedListStyle = params.get(JSON_NL_STYLE, JSON_NL_FLAT).intern();
    ...
    public void writeResponse() throws IOException {
        ...
        if (omitHeader != null && omitHeader) this.values.remove("responseHeader");
        writeNamedList(null, this.values);
        if (wrapperFunction != null) {
            writer.write(')');
        }
